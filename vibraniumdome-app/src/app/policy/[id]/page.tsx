...
import { resetInputShield, resetOutputShield, addInputShield, addOutputShield } from '~/app/store/actions';
...
export default function CreatePolicyPage() {
  ...
  //@ts-ignore
  const inputShieldsData = inputShieldsState.map(item => ({ id: item, shield: item }))
  //@ts-ignore
  const outputShieldsData = outputShieldsState.map(item => ({ id: item, shield: item }))
  ...
  const { data: currentPolicy, isSuccess } = api.policy.get.useQuery({id: policyId}, {
    enabled: !!policyId,
  });
  ...
  //@ts-ignore
  const inputShields = extractShieldsInfo(defaultPolicy?.content?.input_shields)
  //@ts-ignore
  const outputShields = extractShieldsInfo(defaultPolicy?.content?.output_shields)
  ...
  const createPolicyButton = async () => {
    ...
    //@ts-ignore
    const inputShieldsMap = createShieldsMap(defaultPolicy?.content?.input_shields);
    //@ts-ignore
    const outputShieldsMap = createShieldsMap(defaultPolicy?.content?.output_shields);
  ...
  return (
    <>
      <div className="flex-1 space-y-4 p-8 pt-6">
        ...
        <LowRiskThreshold setState={setLowRiskThreshold} defaultValue={lowRiskThreshold}/>
        <HighRiskThreshold setState={setHighRiskThreshold} defaultValue={highRiskThreshold}/>
        <ShieldsFilter setState={setShieldsFilter} defaultValue={shieldsFilter}/>
        <RedactConversation setState={setRedactConversation} defaultValue={redactConversation}/>
      </div>
        ...
        <div className="flex-1 space-y-4 p-8 pt-6">
          <div className="flex items-center justify-between space-y-2">
            ...
            <div className="flex items-center space-x-2">
              <CreateShieldDialog isinput={true} title="Create Input Shield" shields={inputShields}/>
            </div>
          </div>
      ...
      <div className="hidden flex-col md:flex">
        <div className="flex-1 space-y-4 p-8 pt-6">
          <div className="flex items-center justify-between space-y-2">
            ...
            <div className="flex items-center space-x-2">
            <CreateShieldDialog isinput={false} title="Create Output Shield" shields={outputShields}/>
            </div>
          </div>
          <Tabs defaultValue="overview" className="space-y-4">
            <TabsContent value="overview" className="space-y-4">
          ...
          <Button type="submit" onClick={createPolicyButton}>{policyId ? "Update" : "Create"} Policy</Button>
        </div>
      </div>
   </>
 )
